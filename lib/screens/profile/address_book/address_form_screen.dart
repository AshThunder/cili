    ...
    return Observer(
      builder: (_) {
        ...
        Map<String, dynamic>? addressFieldInit =
            widget.type == 'shipping' ? addressData?.shipping : addressData?.billing;

        bool loading = _addressDataStore?.loading != false && addressFieldInit?.isNotEmpty != true;

        Map<String, dynamic> addressFields = widget.type == 'shipping'
            ? {'shipping_address_nickname': fieldNickName, ...addressFieldInit ?? {}}
            : {'billing_address_nickname': fieldNickName, ...addressFieldInit ?? {}};
        return Scaffold(
          ...
          body: loading
              ...
              : addressFieldInit?.isNotEmpty == true
                  ...
                  : _buildaddressEmpty(isShipping: widget.type == 'shipping'),
        );
      },
    );
  }
