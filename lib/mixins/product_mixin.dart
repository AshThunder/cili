...
mixin ProductMixin {
  ...
  Widget? buildQueryData(
    ...
    bool checkField = checkExistQuery(query, languageKey);
    if (checkField) {
      String type = get(query, ["type"], "text");

      if (type == "image") {
        double widthImage = ConvertData.stringToDouble(get(query, ["width"], 100));
        double heightImage = ConvertData.stringToDouble(get(query, ["height"], 100));
        ...
        String? url = getStringQuery(context, product?.rawData, query, languageKey);
        if (url != null) {
      ...
      String? text = getStringQuery(context, product?.rawData, query, languageKey);
      if (text != null) {
        return Text(
          text,
